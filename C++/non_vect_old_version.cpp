...
int main() {
    ...
    while (allF.size() < pop_size) {
        for (int p = 0; p < P.size(); ++p) {
            /*std::find cerca un valore specifico all'interno di un range specificato e restituisce un iteratore 
            che punta all'elemento trovato oppure all'iteratore end() del contenitore se l'elemento non è presente.*/
            if (std::find(allF.begin(), allF.end(), p) != allF.end()) {
                continue;
            }
            int n_p = 0; // domination counter of p (contatore di punti da cui è dominato)
            for (int q = 0; q < P.size(); ++q) {
                if (std::find(allF.begin(), allF.end(), q) != allF.end()) {
                    continue;
                }
                if (domina(P[q], P[p])) {
